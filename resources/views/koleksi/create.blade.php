  ...
  <div class="container pt-4 bg-white">
    <div class="row">
      <div class="col-md-8 col-xl-6">
        ...
        <form action="{{ route('koleksis.store') }}" method="POST" enctype="multipart/form-data">
          @csrf
          <div class="mb-3">
            <label class="form-label" for="jenis_koleksi">Jenis Koleksi</label>
            <input type="text" id="jenis_koleksi" name="jenis_koleksi" value="{{ old('jenis_koleksi') }}"
          ...
          <div class="mb-3">
            ...
            <select class="form-select" id="bibliografi_id" name="bibliografi_id">
                @foreach ($bibliografi as $biblio)
                   ...
                   <option value="{{ $biblio->id }}" {{ old('bibliografi_id')==$biblio->id ? 'selected': '' }}>{{ $biblio->judul }}</option>
                @endforeach
             </select>
